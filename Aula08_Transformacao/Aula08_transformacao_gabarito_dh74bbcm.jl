...
# ╔═╡ fd14c526-36d8-4aed-a4ac-cbf3b4e683e1
md"""
...
# ╔═╡ 3beab858-e412-4165-8da5-6e1bb1a8890e
md"""
**Orientação ao professor — 8.1**

25 °C → 77 °F; 30 °C → 86 °F; a lista [25, 28, 30] produz [77, 82.4, 86]. Compare aproximações de ponto flutuante com tolerância.
"""
...
# ╔═╡ 31d74b04-5fd4-48a3-b464-31f32a87aa3e
md"""
**Orientação ao professor — 8.2**

A expressão nota1+nota2+nota3/3 divide somente a terceira nota. A média pretendida é (nota1+nota2+nota3)/3. O defeito original foi preservado para investigação. [4.5, 6.5, 8.0] produz ["rever", "rever", "ok"]. Valores aleatórios não têm gabarito numérico fixo.
"""
...
# ╔═╡ bcde5699-416a-4312-bb1c-a80e46648a5a
md"""
**Orientação ao professor — 8.3**

A frequência é A: 3, B: 1, C: 1. A ordem de exibição de um Dict não é critério de correção.
"""
...
# ╔═╡ 29ea5641-bf01-4d83-a43e-9088970fe98a
md"""
**Orientação ao professor — 8.4**

O filtro produz [8, 9], pois o critério é valor maior ou igual a 7.
"""
...
# ╔═╡ 3ca43dc4-6ada-41a0-bdad-df8a4affa50f
md"""
**Orientação ao professor — 8.5**

[10,20,30] → [0,0.5,1]. [3,4,4,6,7,5] → [0,0.25,0.25,0.75,1,0.5]. Ao incluir zero, mínimo e intervalo mudam. Intervalo nulo exige uma decisão explícita antes da divisão.
"""
...
# ╔═╡ 81013718-91ba-40d2-91c2-474469ded040
md"""
**Orientação ao professor — 8.6**

Os pares associam A a 10 e B a 20. Dict(trios) é um desafio intencional: cada entrada deve representar chave e valor. Uma alternativa é Dict(nome => (valor, simbolo) for (nome, valor, simbolo) in trios), conservando os dois campos no valor.
"""
...
# ╔═╡ 287d88ee-9f71-482f-a473-097a89b74ed1
md"""
**Orientação ao professor — 8.7**

Os quadrados são [1,4,9,16], e sua soma é 30. Peça que as etapas sejam nomeadas, além do resultado final.
"""
...
# ╔═╡ 5ead9b16-8863-4755-92d8-9891070ae72b
md"""
**Orientação ao professor — 8.8**

A frase original contém 3 palavras. length conta palavras; não calcula suas frequências. Para explorar frequências, pode-se usar Dict(p => count(==(p), palavras) for p in unique(palavras)).
"""
...
# ╔═╡ 77336610-8442-4d49-9abb-d19359962f60
md"""
**Orientação ao professor — 8.9**

O resultado é [false,false,true]. As entradas 1 e 2 tornam-se indistinguíveis; a transformação não permite recuperar a lista original.
"""
...
# ╔═╡ f9cfafad-86ea-4db6-b496-f208b4fde24e
md"""
**Orientação ao professor — 8.10**

Resposta aberta: avaliar correspondência entre pergunta, regra, dados e interpretação. O registro do exemplo descreve uma transformação; não executa a contagem.
"""
...
# ╔═╡ 470db20b-8ace-4fa6-9f0c-40707f44f904
md"""
**Orientação ao professor — 8.11**

A consulta de "dois" retorna "dos"; "quatro" provoca KeyError porque a chave não existe. Discutir haskey, get com uma resposta para ausência, ou cadastrar "quatro" => "cuatro". Preservar a investigação do erro antes de apresentar a solução.
"""
